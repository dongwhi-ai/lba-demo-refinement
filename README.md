...
- 진행사항은 localStorage(`lba-refine-v1`)에 즉시 저장 — 브라우저를 꺼도 유지되고, 기본 URL로 다시 열면 마지막 작업 위치로 복원.
- **xlsx 다운로드**: 언제든 현재 탭 또는 전체 3개를 다운로드. 원본 16열 그대로 + `refinement` 열 기입(채택 → 빈칸, 수정 → `fix, Ab, Ac, 메모`, 폐기 → `del`) + 후미에 `검수상태` 보조열(채택/수정/폐기/미검수, 채택·미검수 구분용 — 불필요하면 열 삭제).
